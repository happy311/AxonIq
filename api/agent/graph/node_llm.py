...
# v18: direct "what are my results / do I have MS" questions during
# post_mri_guidance always get a guaranteed-correct risk headline prepended,
# in code — see api/agent/tools/results_summary.py for why this can't be
# left to LLM instruction-following alone.
_RESULTS_QUESTION_PHRASES = [
    "do i have ms", "do i have multiple sclerosis", "is it ms",
    "what are my results", "what does this mean", "results say",
    "how bad", "risk level", "what is my risk",
    "is report arrived", "report arrived", "results arrived",
    "did the results come", "diagnosis",
]


def _asks_about_own_results(msg: str) -> bool:
    m = msg.lower()
    return any(p in m for p in _RESULTS_QUESTION_PHRASES)
...
def node_llm_response(state: AgentState) -> dict:
    ...
    # ── [critical fix] Short-circuit: node_emergency_check already built a
    # precise, reviewed, deterministic safety message (specific to the exact
    # regex match — stroke vs meningitis vs SAH vs Guillain-Barré, etc.) and
    # set goal="emergency". This must be returned byte-for-byte, verbatim.
    # It must NEVER be regenerated by the LLM (which has no idea which
    # emergency was detected and could hallucinate or soften the guidance)
    # and must NEVER be run through _parse() (the clinical-state extractor
    # meant for the "gathering" goal — an unnecessary, fragile detour for
    # the single highest-stakes response in the app). This check MUST stay
    # first, before every other short-circuit and before any LLM call.
    if goal == "emergency":
        return {
            "response":   state.get("response", ""),
            "next_phase": state.get("next_phase", "concluded"),
        }

    # ── [v18] Short-circuit: mri_analysis just produced a genuine, fully-
    # parsed result (fresh upload/pasted report, or a done recheck) and
    # pre-built a deterministic plain-language summary — see
    # api/agent/tools/results_summary.py. Bypass the LLM entirely for this
    # transition so "your results are in + do I have MS" is never left to a
    # small/unreliable fallback model's instruction-following on the day.
    if state.get("mri_results_ready"):
        preset = state.get("response", "")
        if preset:
            return {
                "response":   preset,
                "next_phase": state.get("next_phase", "mri_received"),
            }
    ...
    # not found / failed). If it came back done, mri_results_ready above
    # already caught it — this handles the not-done outcomes.
    ...
    # ── Build system prompt ───────────────────────────────────────────────────
    # NOTE: goal == "emergency" is handled by the short-circuit above and can
    # never reach this point; EMERGENCY_RESPONSE (imported above) is kept only
    # as an unused historical fallback and is no longer referenced here.
    if goal == "gathering":
    ...
    for attempt in range(MAX_RETRIES + 1):
        try:
            ...
            # ── [fix] Safety-override marker: catches emergency-level symptoms
            # described in ANY goal/phase, not just "gathering" — none of
            # request_mri / conclude_with_mri / mri_received / post_mri_guidance
            # ever reclassify tier otherwise. The LLM's own wording is never
            # trusted for the safety text itself — only the marker is read; the
            # message shown to the patient is always the same deterministic
            # template used by the regex path.
            if "[EMERGENCY_ESCALATION]" in raw:
                logger.warning(
                    "[LLM] SYSTEM_PROMPT safety-override marker fired for goal={}",
                    goal,
                )
                prose = (
                    "⚠️ **Possible Medical Emergency**\n\n"
                    "Based on what you've described, this could be a medical "
                    "emergency that needs immediate attention.\n\n"
                    "Please call emergency services or go to your nearest A&E "
                    "immediately. This cannot be assessed through chat — this is "
                    "a medical emergency."
                )
                return {
                    "response":   prose,
                    "tier":       "CRITICAL_EMERGENCY",
                    "next_phase": "concluded",
                }
    ...
    if goal in _PROSE_ONLY_GOALS:
        ...
        if goal == "post_mri_guidance" and _asks_about_own_results(state.get("user_message", "")):
            from api.agent.tools.results_summary import risk_statement_for_tier
            headline = risk_statement_for_tier(tier)
            if headline.lower() not in prose.lower():
                prose = f"{headline}\n\n{prose}"
        return {
            "response":   prose,
    ...
    # ── [critical fix] CRITICAL_EMERGENCY reached via free-text LLM
    # classification (not the regex fast path) ────────────────────────────────
    # _GATHER_GOAL's tier rules instruct the LLM to classify things like
    # "rapidly worsening bilateral leg weakness", "sudden complete vision
    # loss", "signs of stroke, meningitis, or acute spinal cord compression"
    # as tier CRITICAL_EMERGENCY — a much wider net than the handful of exact
    # phrasings tools/emergency.py's regex catches. But the gathering prompt
    # never tells the LLM to break character when it does this: its prose
    # instructions are "ask ONE question... do NOT write a conclusion", so a
    # patient describing e.g. rapidly worsening bilateral leg paralysis got
    # an ordinary follow-up question this turn. And on the NEXT turn,
    # node_goal_setter treats CRITICAL_EMERGENCY identically to HIGH
    # (idx >= HIGH) and routes to conclude_with_mri, whose prompt has no
    # CRITICAL_EMERGENCY branch — so the patient would be told to see a
    # neurologist and upload an MRI, never to call emergency services.
    # Fix: the moment classification reaches CRITICAL_EMERGENCY, override
    # the LLM's conversational prose with the same deterministic safety
    # message the regex path uses, and end the turn — do not wait for a
    # second round-trip through goal_setter/conclude_with_mri.
    if final_tier == "CRITICAL_EMERGENCY":
        logger.warning(
            "[LLM] CRITICAL_EMERGENCY reached via free-text classification "
            "(not regex) — short-circuiting with canned safety message."
        )
        prose = (
            "⚠️ **Possible Medical Emergency**\n\n"
            "Based on what you've described, this could be a medical "
            "emergency that needs immediate attention.\n\n"
            "Please call emergency services or go to your nearest A&E "
            "immediately. This cannot be assessed through chat — this is a "
            "medical emergency."
        )
        return {
            "response":         prose,
